    ...
    def __init__(
        ...
        if build_index_mappings:
            ...
            if self.shuffle_idx_len != self.sample_idx_len - 1:
                print(
                    f"WARNING: shuffle index length ({self.shuffle_idx_len})"
                    f" is not equal to sample index length ({self.sample_idx_len})"
                )
...
def _build_index_mappings(
    ...
    _filename = data_prefix if index_base_path is None else index_base_path
    ...
    if not use_shared_fs:
        ...
        if torch.distributed.is_initialized():
            ...
            print("WARNING: torch.distributed not initialized. Only run this for testing.")
    ...
    if should_process_dataset:
        if (
            ...
            torch_utils.print_rank_0(
                doc_idx_filename, os.path.isfile(doc_idx_filename),
                sample_idx_filename, os.path.isfile(sample_idx_filename),
                shuffle_idx_filename, os.path.isfile(shuffle_idx_filename),
            )
            # doc-idx.
    ...
    # )
    if torch.distributed.is_initialized():
        # Again, only run in non-distributed mode for testing
        torch.distributed.barrier()

    # Load mappings.
    start_time = time.time()
    torch_utils.print_rank_0(" > loading doc-idx mapping from {}".format(doc_idx_filename))
    doc_idx = np.load(doc_idx_filename, allow_pickle=True, mmap_mode="r")
    torch_utils.print_rank_0(" > loading sample-idx mapping from {}".format(sample_idx_filename))
    sample_idx = np.load(sample_idx_filename, allow_pickle=True, mmap_mode="r")
    torch_utils.print_rank_0(" > loading shuffle-idx mapping from {}".format(shuffle_idx_filename))
    shuffle_idx = np.load(shuffle_idx_filename, allow_pickle=True, mmap_mode="r")
    torch_utils.print_rank_0(
        "    loaded indexed file in {:3.3f} seconds".format(time.time() - start_time)
    )
    torch_utils.print_rank_0("    total number of samples: {}".format(sample_idx.shape[0]))
    torch_utils.print_rank_0("    total number of epochs: {}".format(num_epochs))
