...
def get_model_state_file(checkpoint_dir, zero_stage):
    if not fsspec_isdir(checkpoint_dir):
...
def get_optim_files(checkpoint_dir):
    ...
    optim_files = sorted(fsspec_glob(os.path.join(checkpoint_dir,
                                                  "*_optim_states.pt")),
                         key=natural_keys)
...
def parse_model_state(file):
    state_dict = fsspec_torch_load(file, map_location=device)
...
def parse_optim_states(files, ds_checkpoint_dir):
    ...
    for f in tqdm.tqdm(files):
        state_dicts.append(fsspec_torch_load(f, map_location=device))
...
def get_fp32_state_dict_from_zero_checkpoint(checkpoint_dir, tag=None):
    ...
    if tag is None:
        ...
        if fsspec_isfile(latest_path):
            with fsspec.open(latest_path, mode="r") as fd:
                tag = fd.read().strip()
        else:
    ...
    if not fsspec_isdir(ds_checkpoint_dir):
...
def convert_zero_checkpoint_to_fp32_state_dict(checkpoint_dir, output_file, fp16, bf16, tag=None):
    ...
    fsspec_torch_save(state_dict, output_file)
